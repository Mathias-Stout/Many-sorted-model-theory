...
/-A trick suggested by Sky Wilshaw.
  An alternative would be to work with a variable set +
  assigment functions instead of sorted tuples everywhere-/
...
/-- Helper simp lemma for applying MSEquiv inverse on the left. -/
@[simp] lemma inv_to (e : M ≃ₛ N) (t : Sorts) (x : M t) :
    e.invFun t (e.toFun t x) = x :=
  (e.left_inv' t) x

/-- Helper simp lemma for applying MSEquiv inverse on the right. -/
...
@[coe]
def PerSortEquivLike.toEquiv {Sorts : Type _} {F} {M N : Sorts → Type _}
    [DFunLike F Sorts (fun t => M t → N t)] [PerSortEquivLike F M N] (f : F) : M ≃ₛ N where
  toFun := f
