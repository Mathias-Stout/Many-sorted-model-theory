...
/-- A type for the Fam of pointwise maps between two Fams. -/
...
@[simp] lemma sumElim_eval_l (f : α →ₛ γ) (g : β →ₛ γ) {s : base} {v : α s} :
    (sumElim f g) s (Sum.inl v) = f s v := by
  simp only [sumElim, FamMap.mk_apply, Sum.elim_inl]

@[simp] lemma sumElim_eval_r (f : α →ₛ γ) (g : β →ₛ γ) {s : base} {v : β s} :
    (sumElim f g) s (Sum.inr v) = g s v := by
  simp only [sumElim, FamMap.mk_apply, Sum.elim_inr]
...
@[simp] lemma sumMap_id :
  ...
  ext s x;
  cases x <;>
  simp_all only [FamMap.idₛ_apply'] <;>
  rfl
...
instance emptyDomInhabitedMap [∀ (s : base), IsEmpty (α s)] : Inhabited (α →ₛ β) := by
  apply Inhabited.mk
  use fun s (v : α s) => IsEmpty.elim' (by infer_instance) v
