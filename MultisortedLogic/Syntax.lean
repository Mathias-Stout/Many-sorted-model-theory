...
-/
inductive Term (L : MSLanguage.{u, v, z} Sorts) (α : Fam.{u'} Sorts) :
...
abbrev Term₁ (L : MSLanguage.{u, v, z} Sorts) (α : Fam.{u'} Sorts) : Fam Sorts :=
  ⟨fun s => L.Term α ⦃s⦄⟩
...
theorem rename_id (t : L.Term (α ⊕ₛ γ) σ) : t.rename (Fam.FamMap.idₛ) = t := by
  simp only [rename, mapVars, sumMap, FamMap.idₛ, FamMap.mk_apply, Sum.map_id_id, id_eq, bind_var]

@[simp]
theorem rename_rename (t : L.Term (α ⊕ₛ γ) σ) (f : α →ₛ β) (g : β →ₛ δ) :
  (t.rename f).rename g = t.rename (g ∘ₛ f) := by
  simp only [rename]
  simp_all only [mapVars, FamMap.mk_apply, bind_bind, bind, sumMap,Sum.map_map]
  rfl

@[simp]
theorem reindex_id (t : L.Term (α ⊕ₛ σ.IdxFam) τ) : t.reindex FamMap.idₛ = t := by
  simp only [reindex, mapVars, FamMap.idₛ, sumMap]
  simp_all only [FamMap.mk_apply, Sum.map_id_id, id_eq, bind_var]
...
instance IdxFamofInhabited {s : Sorts} : Inhabited (IdxFam (of s) s) := by
  constructor
  case default =>
    exact Idx.var
...
theorem rename_subst (t : Term L (α ⊕ₛ σ.IdxFam) τ)
  ...
  cases a with
  | inl val =>
  simp_all only [sumMap_inl_apply, sumElim_eval_l]
  rfl
  | inr val_1 =>
  simp_all only [sumMap_inr_apply, FamMap.idₛ_apply', sumElim_eval_r]
...
lemma close_openVars
  ...
  | var s a =>
      ...
      | inl val =>
        cases val with
        | inl val_1 =>
          simp_all only [reindex_var_inl, subst_var_inl, rename_var_inl]
          rfl
        | inr
          val_2 =>
          simp_all only [reindex_var_inl, subst_var_inl, rename_var_inl]
...
abbrev Formula (L : MSLanguage.{u, v, z} Sorts) (α : Fam.{u'} Sorts) := BoundedFormula L α nil
...
variable {L : MSLanguage.{u, v, z} Sorts} {α : Fam.{u'} Sorts} {β : Sorts → Type v'}
...
@[simp]
theorem mapTermRel_mapTermRel {β : Fam Sorts} {L'' : MSLanguage Sorts}
...
lemma openVars_closeVars {σ τ : Signature Sorts} (φ : L.BoundedFormula α (σ ⨯ τ)) :
  ...
  | rel R ts =>
    ...
    cases a
    · simp_all only [Sum.elim_inl, Term.bind, FamMap.mk_apply]
      rfl
      ...
      { congr 2
        ...
        · case inl w =>
          ...
          · simp_all only [Term.bind, FamMap.mk_apply, FamMap.comp_apply']
            rfl
          · simp_all only [Term.bind]; rfl
        · case inr =>
        simp_all only [Term.bind]
        rfl;
      }
  | @rel σ' τ' R ts =>
...
theorem equivSentence_not (φ : L.Formula α) : equivSentence φ.not = (equivSentence φ).not :=
  by
    simp only [equivSentence, BoundedFormula.constantsVarsEquiv, BoundedFormula.mapTermRelEquiv,
      constantsOn_Relations, BoundedFormula.relabelEquiv, Equiv.coe_refl, Equiv.refl_symm,
      BoundedFormula.not, Equiv.symm_trans_apply, Equiv.coe_fn_symm_mk, BoundedFormula.mapTermRel,
...
theorem equivSentence_imp (φ ψ : L.Formula α) :
    ...
    simp only [equivSentence, BoundedFormula.constantsVarsEquiv, BoundedFormula.mapTermRelEquiv,
      ...
      BoundedFormula.mapTermRel_mapTermRel, CompTriple.comp_eq]
