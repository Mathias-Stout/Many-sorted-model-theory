import MultisortedLogic.SyntaxClasses
...
variable {M : Fam.{w} Sorts} {N : Fam Sorts} {P : Fam Sorts} [i : L.Structure M]
  [L.Structure N] [L.Structure P]
variable {α : Fam.{u'} Sorts} {β : Fam.{v'} Sorts} {γ : Fam Sorts}
variable {s : Sorts} {t : Sorts}
namespace Language
open MSFirstOrder Cardinal

open Structure Language Fin Finsupp Fam Signature
open Interpret
...
lemma realize_comap
  ...
  congr!
  ext s v
  cases v <;> first | rfl | exact congrFun (DFunLike.congr_fun (get_comap xs g) s) _
...
theorem realize_instantiate {η ρ σ : Signature Sorts}
  ...
  | var s x =>
      ...
      | inr w =>
          cases w with
          | left w => rfl
          | right w => exact (realize_getLeafTerm u (Fam.sumElim v xs.get) s w).symm
  | func g ts ih =>
      rw[instantiate] at *
...
| func σ g ih =>
    ...
    apply @hv'
| prod t₁ t₂ ih₁ ih₂ =>
  exact congrArg₂ Prod.mk (ih₁ _ fun {s} a => hv' _) (ih₂ _ fun {s} a => hv' _)
| nil => simp_all only
...
theorem realize_restrictVarLeft
  ...
  | var s a =>
      ...
      · simp_all only [sumElim_eval_l]
        apply @hxs'
      · simp_all only [sumElim_eval_r, FamMap.mk_apply]
        rfl
  | func σ g ih =>
      ...
      apply ih f
      intro s a
      simp_all only [varFamLeft, varFinsetLeft]
      apply @hxs'
  | prod t₁ t₂ ih₁ ih₂ =>
      exact congrArg₂ Prod.mk (ih₁ _ fun {s} a => hxs' _) (ih₂ _ fun {s} a => hxs' _)
  | nil => simp_all only
...
theorem realize_constantsToVars
  [L[[α]].Structure M] [(lhomWithConstants L α).IsExpansionOn M]
...
@[simp]
theorem realize_onTerm {σ : Signature Sorts} [L'.Structure M] (φ : L →ᴸ L')
...
theorem realize_mapTermRel_id [L'.Structure M] {φ : L.BoundedFormula α σ}
...
theorem realize_restrictFreeVar [DecidableEq Sorts] [∀ s, DecidableEq (α s)]
  ...
  | @equal σ τ t₁ t₂ =>
    ...
    have h1 : (t₁.restrictVarLeft ⟨fun {t} x =>
      ...
      · congr 1
      · intro s ⟨a, ha⟩
        simp_all only [freeVarFinset, Finset.mem_union, FamMap.mk_apply,
          sumElim_eval_l]
        apply hv
...
theorem realize_mapTermRel_add_reindex
  [L'.Structure M] {σ τ ξ : Signature Sorts}
...
@[simp]
theorem realize_onBoundedFormula [L'.Structure M] (φ : L →ᴸ L') [φ.IsExpansionOn M]
...
theorem realize_ex_root {α : Fam.{u'} Sorts} {φ : L.BoundedFormula α (⦃⦄ ⨯ ⦃s⦄)}
    {M : Fam.{w} Sorts} {v : α →ₛ M} [L.Structure M] :
...
theorem realize_fully_instantiate {τ : Signature Sorts}
  ...
  congr!; ext s v; simp_all only [reduce_nil, get_comap]; rfl
...
@[simp]
theorem LHom.realize_onFormula [L'.Structure M] (φ : L →ᴸ L') [φ.IsExpansionOn M]
...
-- Note: The Unique instance on maps of type ((EmptyFam ⊕ₛ ⦃⦄.IdxFam) →ₛ β) is coming from the
-- emptyDomUniqueMap instance in Fam.lean
@[simp]
theorem Sentence.realize_equals {t₁ t₂ : L.Term (EmptyFam ⊕ₛ ⦃⦄.IdxFam) σ} :
  M ⊨ t₁.bdEqual t₂ ↔ (t₁.realize (M:= M) default = t₂.realize default) := by
  rw [Realize, Formula.Realize, PUnit.default_eq_unit, BoundedFormula.realize_bdEqual,
    Interpret.get]
...
theorem Sentence.realize_rel {r : L.Relations σ} {t : L.Term (EmptyFam ⊕ₛ ⦃⦄.IdxFam) σ} :
  ...
  rw [Realize, Formula.Realize, BoundedFormula.Realize, PUnit.default_eq_unit]
  simp only [Unique.uniq]
...
@[simp] theorem realize_toFormula
  ...
  apply realize_restrictFreeVar
  intro s a h
  exact congrArg (v s) (congrArg Subtype.val (MSEquiv.inv_to lf.e s ⟨a, h⟩))
...
@[simp] lemma fromGet_comap {M : Fam.{w} Sorts} [L.Structure M]
...
theorem realize_toBoundedFormula
  ...
  φ.Realize x ↔
    ...
    · intro s a h
      ...
      exact congrArg Subtype.val (MSEquiv.inv_to lf.e s ⟨a, h⟩)
...
theorem realize_toFormula_formula
  ...
  φ.Realize x ↔
    ...
    intro s a h
    exact congrArg (x s) (congrArg Subtype.val (MSEquiv.inv_to lf.e s ⟨a, h⟩))
...
@[inherit_doc MSFirstOrder.Language.ElementarilyEquivalent]
scoped[MSFirstOrder]
  notation:25 A " ≅[" L "] " B:50 => MSFirstOrder.Language.ElementarilyEquivalent L A B
...
@[simp]
theorem _root_.MSFirstOrder.Language.Formula.realize_iAlls
...
@[simp]
theorem _root_.MSFirstOrder.Language.Formula.realize_iExs
    [Finite (Sigma β)] {φ : L.Formula (α ⊕ₛ β)} {v : α →ₛ M} :
    (φ.iExs β).Realize v ↔
      ∃ (i : β →ₛ M), φ.Realize (Fam.sumElim v i) := by
  ...
  · rintro ⟨i, hi⟩
    ...
    case inr a =>
      ...
      exact (congrArg (i s) (MSEquiv.inv_to e s a)).symm
...
/-- A BoundedFormula is realized if and only if its corresponding `toFormula` form is. -/
theorem realize_iff_toFormula (φ : L.BoundedFormula α σ) (v : α →ₛ M) (x : M [^] σ) :
    φ.Realize v x ↔ φ.toFormula.Realize (Fam.sumElim v x.get) := by
  rw [toFormula, Formula.Realize, realize_openVars, realize_reindex, comap]
  congr!
  ext s v
  simp only [PUnit.default_eq_unit, SigMap.nil_left_inv, SigMap.incl_right_apply, get_right,
    fromGet_get, FamMap.mk_apply]
...
theorem realize_constantsVarsEquiv {γ : Fam Sorts}
    [L[[γ]].Structure M] [(lhomWithConstants L γ).IsExpansionOn M]
  ...
  rcongr
  obtain - | R := R
  · simp only [lhomWithConstants_onRelation]
    rfl
  · exact isEmptyElim R
...
@[simp]
theorem _root_.MSFirstOrder.Language.Formula.realize_iSup {X} [Finite X] {f : X → L.Formula α}
...
@[simp]
theorem _root_.MSFirstOrder.Language.Formula.realize_iInf {X} [Finite X] {f : X → L.Formula α}
...
theorem _root_.MSFirstOrder.Language.Formula.realize_iExsUnique {X : Fam Sorts} [Finite (Sigma X)]
...
theorem realize_equivSentence_symm (φ : L[[α]].Sentence) (v : FamMap α M) :
    (equivSentence.symm φ).Realize v ↔
      ...
      (@Language.withConstantsStructure Sorts L M _ α (constantsOn.structure v))
        φ :=
  ...
  | equal t₁ t₂ =>
    ...
    have hmap :
        StrongEquivHomClass.inv g <$>ₛ (g <$>ₛ ys)
          ...
          rw [Interpret.comp_map, mapClass_eq_map]
          rfl
    calc
      StrongEquivHomClass.inv g <$>ₛ (g <$>ₛ ys)
  ...
  | all η φ ih =>
    ...
    · intro h ys
      ...
      have h'' : φ.Realize ((g : M →ₛ N) ∘ₛ v) (g <$>ₛ (xs, ys)) := by
        rw [Interpret.map_prod, mapClass_eq_map]
...
@[simp]
theorem realize_symmetric : M ⊨ r.symmetric ↔ Std.Symm fun x y : M s => i.RelMap r ⟨x, y⟩ := by
...
theorem realize_total : M ⊨ r.total ↔ Std.Total fun x y : M s => i.RelMap r ⟨x, y⟩ := by
  ...
  · intro h
    constructor
    intro a b
    simp only [Interpret, reduce_nil, PUnit.default_eq_unit, VarIndex.get] at h
    tauto
  · intro h a
    simp only [Interpret, reduce_nil, PUnit.default_eq_unit, realize_reindex, VarIndex.get]
    intro x h'
    let h'' := h.total x a
    tauto
...
@[simp] lemma realize_distinct {n : ℕ}
  ...
  | succ n ih =>
    ...
    simp only [get_comap, SigMap.incl_left_apply, get_left, realize_distinct_from L v (xs', x)]
    constructor
    ...
    · intro hInj
      ...
      · intro i j hij
        ...
        exact Idx.left_injEq.mp (this hij)
      · intro i hEq
...
theorem model_distinctConstantsAtSortTheory {M : Fam Sorts} [L[[α]].Structure M] (S : Set (α s)) :
...
theorem card_le_of_model_distinctConstantsAtSortTheory (S : Set (α s)) (M : Fam.{w} Sorts)
    [L[[α]].Structure M] [h : M ⊨ L.distinctConstantsAtSortTheory s S] :
...
theorem model_distinctConstantsTheory {M : Fam Sorts} [L[[α]].Structure M] (S : DepSet α) :
...
theorem card_le_of_model_distinctConstantsTheory (S : DepSet α) (M : Fam.{w} Sorts)
    [L[[α]].Structure M] [h : M ⊨ L.distinctConstantsTheory S] :
