...
def Fam.sumElim {γ : S → Type*} (f : α →ₛ γ) (g : β →ₛ γ) : (α ⊕ₛ β) →ₛ γ :=
...
@[simp]
theorem Fam.sumComp_elim {γ : S → Type*} {δ : S → Type*} (f : γ →ₛ δ) (g : α →ₛ γ) (h : β →ₛ γ) :
    (f ∘ₛ (Fam.sumElim g h)) = Fam.sumElim (f ∘ₛ g) (f ∘ₛ h) := by
  funext s x
...
def realize {t : Sorts} (v : α →ₛ M) : L.Term α t →  M t
  | var t k => v t k
...
@[simp]
theorem realize_func (v : α →ₛ M) {σ} (f : L.Functions σ t) (ts) :
    realize v (func σ t f ts : L.Term α t) = funMap f fun i => (ts i).realize v := rfl

@[simp]
theorem realize_function_term {σ} (v : σ.toFam →ₛ M) (f : L.Functions σ t) :
    f.term.realize v = funMap f (SortedTuple.fromFMap v) := by
  rfl

@[simp]
theorem realize_relabel {t : L.Term α s} {g : α →ₛ β} {v : β →ₛ M} :
    (t.relabel g).realize v = t.realize (v ∘ₛ g) := by
  induction t with
  | var => rfl
  | func σ t f ts ih => simp [ih]
...
@[simp]
theorem realize_constants {c : L.Constants t} {v : α →ₛ M} : c.term.realize v = c :=
  funMap_eq_coe_constants

@[simp]
theorem realize_functions_apply₁ {f : L.Functions [s] t} {g : L.Term α s} {v : α →ₛ M} :
    (f.apply₁ g).realize v = funMap f !ₛ[⟨s,g.realize v⟩] := by
...
theorem realize_con [DecidableEq Sorts] {A : (s : Sorts) → Set (M s)} {s : Sorts} {a : A s}
  --We help lean to synthesize the correct `ConstantsOn' structure (namely the L[[M]] - structure)
    {v : α →ₛ M} : (L.con (α := M) s a).term.realize v = a :=
  rfl

@[simp]
theorem realize_subst {t : L.Term α s} {tf : α →ₛ (L.Term β)} {v : β →ₛ M} :
...
@[simp]
theorem realize_onTerm [L'.MSStructure M] (φ : L →ᴸ L') [φ.IsExpansionOn M] (t : L.Term α s)
    (v : famMap α M) : (φ.onTerm t).realize v = t.realize v := by
  induction t with
  | var => rfl
  | func _ _ f ts ih => simp only [Term.realize, LHom.onTerm, LHom.map_onFunction, ih]
...
theorem HomClass.realize_term {F : Type*} [DFunLike F Sorts (fun t => M t → N t)] [HomClass L F M N]
    (g : F) {s : Sorts} {t : L.Term α s} {v : famMap α M} :
    t.realize (g ∘ₛ v) = g s (t.realize v) := by
  induction t
  · rfl
...
def Realize : ∀ {ξ} (_f : L.BoundedFormula α ξ) (_v : α →ₛ M) (_xs : SortedTuple ξ M), Prop
  | _, falsum, _v, _xs => False
  | _, equal t₁ t₂, v, xs => t₁.realize (Fam.sumElim v xs)
      = t₂.realize (Fam.sumElim v xs)
  | _, rel R ts, v, xs => RelMap R (fromMap fun i => (ts i).realize (Fam.sumElim v xs))
...
@[simp]
theorem realize_bdEqual (t₁ t₂ : L.Term (α ⊕ₛ ξ.toFam) s) :
...
theorem realize_foldr_imp {η : List Sorts} (l : List (L.BoundedFormula α η))
...
@[simp]
theorem realize_rel₁ {s : Sorts} {R : L.Relations [s]} {t : L.Term _ _} :
    (R.boundedFormula₁ t).Realize v xs ↔ RelMap R !ₛ[⟨s, t.realize (Fam.sumElim v xs)⟩] := by
...
theorem realize_rel₂ {s₁ s₂ : Sorts} {R : L.Relations [s₁,s₂]}
  ...
  rw [Relations.boundedFormula₂, realize_rel, iff_eq_eq]
  simp_all only [List.length_cons, List.length_nil, Nat.reduceAdd, List.get_eq_getElem, eq_iff_iff]
  rfl
...
@[simp]
theorem realize_all : (all θ).Realize v xs ↔ ∀ a : M r, θ.Realize v (extend xs a) :=
  Iff.rfl

@[simp]
theorem realize_ex : (θ.ex).Realize v xs ↔ ∃ a : M r, θ.Realize v (extend xs a) := by
...
theorem realize_mapTermRel_id [L'.MSStructure M]
    {ft : ∀ σ, famMap (L.Term (addSigVars α σ)) (L'.Term (addSigVars β σ))}
    {fr : ∀ σ, L.Relations σ → L'.Relations σ} {σ} {φ : L.BoundedFormula α σ} {v : famMap α M}
  ...
  | all _ _ ih => simp only [mapTermRel, Realize, ih]
-/
--todo: when required
/-
theorem realize_mapTermRel_add_castLe [L'.MSStructure M] {k : ℕ}
    {ft : ∀ n, L.Term (α ⊕ (Fin n)) → L'.Term (β ⊕ (Fin (k + n)))}
    {fr : ∀ n, L.Relations n → L'.Relations n} {n} {φ : L.BoundedFormula α n}
    (v : ∀ {n}, (Fin (k + n) → M) → famMap α M) {v' : famMap β M} (xs : Fin (k + n) → M)
    (h1 :
      ∀ (n) (t : L.Term (α ⊕ (Fin n))) (xs' : Fin (k + n) → M),
        (ft n t).realize (Sum.elim v' xs') = t.realize (Sum.elim (v xs') (xs' ∘ Fin.natAdd _)))
    (h2 : ∀ (n) (R : L.Relations n) (x : sorted_tuple σ M), RelMap (fr n R) x = RelMap R x)
...
theorem realize_restrictFreeVar [DecidableEq α] {n : ℕ} {φ : L.BoundedFormula α n}
    {f : φ.freeVarFinset → β} {v : famMap β M} {xs : sorted_tuple σ M}
...
theorem realize_restrictFreeVar' [DecidableEq α] {n : ℕ} {φ : L.BoundedFormula α n} {s : Set α}
    (h : ↑φ.freeVarFinset ⊆ s) {v : famMap α M} {xs : sorted_tuple σ M} :
...
theorem realize_constantsVarsEquiv [L[[α]].MSStructure M] [(lhomWithConstants L α).IsExpansionOn M]
    {n} {φ : L[[α]].BoundedFormula β n} {v : famMap β M} {xs : sorted_tuple σ M} :
...
theorem realize_all_liftAt_one_self {n : ℕ} {φ : L.BoundedFormula α n} {v : famMap α M}
    {xs : sorted_tuple σ M} : (φ.liftAt 1 n).all.Realize v xs ↔ φ.Realize v xs := by
  inhabit M
...
theorem realize_rel₁ {R : L.Relations [s]} {t : L.Term α s} :
    (R.formula₁ t).Realize v ↔ RelMap R !ₛ[⟨s, t.realize v⟩] := by
...
theorem realize_relabel_sumInr (φ : L.Formula (Fin n)) {v : Empty → M} {x : sorted_tuple σ M} :
...
@[simp]
theorem realize_equal {t₁ t₂ : L.Term α s} {x : famMap α M} :
    (t₁.equal t₂).Realize x ↔ t₁.realize x = t₂.realize x := by simp [Term.equal, Realize]
/-
@[simp]
theorem realize_graph {σ} {f : L.Functions σ s} {x : sorted_tuple σ M} {y : M s} :
...
theorem boundedFormula_realize_eq_realize (φ : L.Formula α) (v : α →ₛ M) (ys : SortedTuple [] M) :
...
theorem realize_alls {φ : L.BoundedFormula α σ} {v : α →ₛ M} :
    φ.alls.Realize v ↔ ∀ xs : sorted_tuple σ M, φ.Realize v xs := by
  induction n with
...
theorem realize_exs {φ : L.BoundedFormula α n} {v : famMap α M} :
    φ.exs.Realize v ↔ ∃ xs : sorted_tuple σ M, φ.Realize v xs := by
  induction n with
...
theorem realize_iSup [Finite β] {f : β → L.BoundedFormula α n}
    {v : famMap α M} {v' : sorted_tuple σ M} :
...
theorem realize_boundedFormula {σ} (φ : L.BoundedFormula α σ) {v : α →ₛ M}
    {xs : SortedTuple σ M} : φ.Realize (g ∘ₛ v) (g <$>ₛ xs) ↔ φ.Realize v xs := by
  induction φ with
  | falsum => rfl
  | equal =>
    rw [BoundedFormula.Realize,BoundedFormula.Realize, SortedTuple.toFMap_map,
    ←  Fam.sumComp_elim g v (xs.toFMap), HomClass.realize_term,
        HomClass.realize_term]
    refine Function.Injective.eq_iff (Function.HasLeftInverse.injective ?_)
    exact ⟨((PerSortEquivLike.inv g) _), PerSortEquivLike.left_inv g _⟩
  | rel =>
    rename_i σ' σ R ts
    rw [BoundedFormula.Realize, BoundedFormula.Realize, SortedTuple.toFMap_map,
         ← Fam.sumComp_elim g v xs.toFMap ]
    have : (fun i => Term.realize (fun t ↦ g t ∘ Fam.sumElim v xs.toFMap t) (ts i)) =
        fun i => g (σ.get i) (Term.realize (fun t ↦ Fam.sumElim v xs.toFMap t) (ts i)) := by
        funext i
        rw [HomClass.realize_term]
    rw [this, SortedTuple.fromMap_map]
  ...
  | @all _ s φ ih3 =>
    ...
    · intro h a
      have h' := h (g s a)
      rw [← SortedTuple.map_extend, ih3] at h'
      exact h'
    · intro h a
      have h' := h (PerSortEquivLike.inv g s a)
      rw [← ih3, SortedTuple.map_extend, PerSortEquivLike.right_inv g] at h'
      exact h'
